...
const TURNSTILE_RETRY_DELAY = 250; // milliseconds
...
function initializeTurnstileWidgets() {
  if (!window.turnstile) {
    if (turnstileRetryCount < TURNSTILE_MAX_RETRIES) {
      turnstileRetryCount += 1;
      window.setTimeout(initializeTurnstileWidgets, TURNSTILE_RETRY_DELAY);
    } else {
      if (!turnstileWarningLogged) {
        turnstileWarningLogged = true;
        console.error('Cloudflare Turnstile has not loaded yet. Retrying with a longer delay.');
      }
      window.setTimeout(initializeTurnstileWidgets, TURNSTILE_RETRY_DELAY * 4);
    }
    return;
  }
...
function queueTurnstileInitialization() {
  if (turnstileReady) {
    mountTurnstileWidgets();
    return;
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', queueTurnstileInitialization, { once: true });
} else {
  queueTurnstileInitialization();
}
