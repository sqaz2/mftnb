...
function getTurnstileSecret() {
  if (!cachedTurnstileSecret) {
    const props = PropertiesService.getScriptProperties();
    cachedTurnstileSecret = props.getProperty('TURNSTILE_SECRET');
    if (!cachedTurnstileSecret) {
      throw new Error('TURNSTILE_SECRET script property is not set. Add it under File → Project properties → Script properties.');
    }
  }
...
function handleEstimateSubmission(body) {
  ...
  const rooms = body.homeSize || body.bedrooms || '';
  const row = [
    timestamp,
    body.name || '',
    body.email || '',
...
function verifyTurnstile(token, e, expectedAction) {
  ...
  try {
    ...
    console.error('Turnstile secret missing', err);
    throw err;
  }
  const payload = {
    secret,
  ...
  return `
    ...
    <table border="0" cellpadding="6" cellspacing="0" style="border-collapse:collapse;">
      <tbody>
        ...
        <tr><td><strong>Home type</strong></td><td>${sanitize(homeType)}</td></tr>
        <tr><td><strong>Bedrooms / main rooms</strong></td><td>${sanitize(rooms)}</td></tr>
...
function buildCustomerEstimateHtml(body, timestamp) {
  ...
  const homeType = body.homeType || '';
  const rooms = body.homeSize || body.bedrooms || '';
  const homeSummary = [homeType, rooms].filter(Boolean).join(' · ') || 'Details pending';
  return `
    <p>Hi ${sanitize(body.name)},</p>
    ...
    <ul>
      ...
      <li><strong>Home type &amp; rooms:</strong> ${sanitize(homeSummary)}</li>
